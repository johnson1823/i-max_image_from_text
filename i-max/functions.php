...
function show_image_from_txt($post_id) {
	$text = get_post( $post_id );

	if( strpos($text->post_content, "<img") !== false) {
		preg_match_all('@src="([^"]+)"@', $text->post_content, $result); 
		
		$wylosuj = count($result[0])-1;
		$losowanie = rand(0, $wylosuj);
		
		return $result[1][$losowanie];//[$losowanie][0];
	} else {
		return false;
	}
}
